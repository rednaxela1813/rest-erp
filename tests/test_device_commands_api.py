...
@pytest.mark.django_db
def test_device_commands_pull_respects_limit(admin_client, payment_factory, capture_payment_api):
    client, user, org = admin_client
    ...
    pull = client.get("/api/v1/device/commands/pull/?limit=2")
    assert pull.status_code == 200, pull.content
    payload = pull.json()

    assert len(payload) == 2
    pulled_ids = [row["public_id"] for row in payload]
    assert DeviceCommand.objects.filter(public_id__in=pulled_ids, status=DeviceCommand.Status.SENT).count() == 2
